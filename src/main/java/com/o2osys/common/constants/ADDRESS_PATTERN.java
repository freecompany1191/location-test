...
public interface ADDRESS_PATTERN {
    ...
    public enum DEFAULT {
        ...
        ,DUPLE_SIGUGUN(Pattern.compile("([가-힣]+(시|도)|"+SIDO+")\\s[가-힣]+(시|군|구)\\s(([가-힣]{1,5})+("+DONG_NUM+"|)+(읍|면|동|가|리|로|길))?"))
        /** STEP5 -OO동999-999 or 002.3동999-999 or OO2,3동999-999 */
        ,DONG_BUNGI(Pattern.compile("([가-힣]{1,5})+("+DONG_NUM+"|)+(읍|면|동|가|리)+(?=("+BUNGI_NUM+")+(번지|))"))
        /** STEP6 - 아파트와숫자 or 아파트가동 or 아파트A동 or 아파드726번지 or 아파트108호 패턴이 붙어있을 시 아파트 이름만 가져옴 */
        ,APT_BLANK(Pattern.compile("([가-힣](\\d{1,5})?)+("+APT_LIST+")+(?=(\\d{1,5}|\\w|[가-힣])+(동|번지|호))"))
        /** STEP7 - 999번지 or 999-999번지에 공백 이외의 문자가 붙어있을때 번지만 가져옴 */
        ,BUNGI_BLANK(Pattern.compile("("+BUNGI_NUM+")+(번지)+(?=(\\w|[가-힣]|\\d{1,5}))"))
        /** STEP8 - OO동 or 002.3동 or OO2,3동 중복동 제거*/
        ,DUPLE_DONG(Pattern.compile("([가-힣]{1,5})+("+DONG_NUM+"|)+(읍|면|동|가|리)\\s"))
        /** STEP9 - 블랭크 제거 */
