...
public interface ADDRESS_PATTERN {
    ...
    String APT_LIST =
            "아파트|APT|스카이뷰|파크|자이|파크|아이파크|편한세상|래미안|푸르지오|프레스티지|뉴스테이|단지|차|"
    ...
    public enum DEFAULT {
        ...
        ,APT_BLANK(Pattern.compile("(([가-힣]\\d{1,5}|[가-힣])?)+(\\s?)+("+APT_LIST+")+(?=(\\d{1,4}|\\w|"+APT_KOR_DONG_STR+")+(동|번지|호))"))
        ...
        ,DUPLE_DONG(Pattern.compile("\\s((?!"+APT_DONG+")([가-힣]{1,5})+("+DONG_NUM+"|)+동)"))
