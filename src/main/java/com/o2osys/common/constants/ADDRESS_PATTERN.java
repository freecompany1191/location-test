...
public interface ADDRESS_PATTERN {
	...
	public enum DEFAULT {
	 ...
	 ,DONG_BUNGI(Pattern.compile("([가-힣]{1,5})+("+DONG_NUM+"|)+(읍|면|동|가|리)\\d{1,5}(~|-)\\d{1,5}", Pattern.DOTALL))
	 /** 999-999 or 999~999 */
	 ,BUNGI(Pattern.compile("\\d{1,5}(~|-)\\d{1,5}"))
	 /** 아파트와숫자 or 아파트가동 or 아파트A동 패턴이 붙어있을 시 아파트 이름만 가져옴 */
	 ,APT_BLANK(Pattern.compile("([가-힣](\\d{1,5})?)+("+APT_LIST+")+(?=\\d{1,5}|\\w|([가-힣]동))"))
